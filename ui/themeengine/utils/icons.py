class Icon:
    """Clase contenedora que almacena imágenes codificadas en base64 para ser utilizadas
    en la clase `PhotoImage` de Tkinter mediante el parámetro `data`.

    Esta clase proporciona íconos estándar listos para usar sin necesidad de archivos
    externos, facilitando la distribución de aplicaciones con una interfaz gráfica
    consistente y evitando problemas de rutas relativas.

    Atributos:
        icon (str): Ícono principal de la aplicación ttkbootstrap.
        error (str): Imagen que representa un error o alerta crítica.
        warning (str): Imagen que representa una advertencia.
        question (str): Imagen que representa una pregunta o solicitud de información.
        info (str): Imagen que representa un mensaje informativo.

    Ejemplos:
        Para crear un objeto PhotoImage con el ícono de advertencia:

        ```python
        img = tkinter.PhotoImage(data=Icon.warning)
        label = tkinter.Label(root, image=img)
        ```

        Para establecer el ícono de una ventana:

        ```python
        root = tkinter.Tk()
        icon = tkinter.PhotoImage(data=Icon.icon)
        root.iconphoto(True, icon)
        ```
    """

    # Datos de íconos en formato base64
    # Ícono principal de la aplicación - Se utiliza como ícono predeterminado
    # para ventanas principales y como respaldo cuando no se encuentra un ícono personalizado
    ...
    # Ícono de error - Utilizado para diálogos de error y notificaciones críticas
    # Típicamente rojo con una 'X' o símbolo similar
    ...
    # Ícono de advertencia - Utilizado para alertas y mensajes preventivos
    # Típicamente amarillo/ámbar con signo de exclamación
    ...
    # Ícono de pregunta - Utilizado para diálogos de confirmación o solicitud de información
    # Típicamente azul con signo de interrogación
    ...
    # Ícono de información - Utilizado para mensajes informativos no críticos
    # Típicamente azul con la letra 'i' o símbolo similar
